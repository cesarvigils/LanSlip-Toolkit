...
const LOG_CHANNEL_ID = '1533580186291933234';
const SUBMIT_ROLE_ID = '1533902868971524286';   // role required to run /recruitmentlog
...
async function ensureTable() {
    ...
    await db.query(`
        CREATE TABLE IF NOT EXISTS recruitment_logs (
            ...
            recruit_id VARCHAR(32) NOT NULL,
...
function buildEmbed({ recruit, submitter, status, decidedBy }) {
    return new EmbedBuilder()
        ...
        .addFields(
            { name: 'Recruit', value: `<@${recruit.id}>` },
            { name: 'Recruited By', value: `<@${submitter.id}>` },
    ...
    async execute(interaction) {
        ...
        try {
            ...
            await interaction.editReply({
                content: 'Recruitment log submitted for approval.',
                embeds: [pendingEmbed],
            });
            ...
            const row = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('recruitmentlog_approve').setLabel('Approve').setStyle(ButtonStyle.Success),
                new ButtonBuilder().setCustomId('recruitmentlog_reject').setLabel('Reject').setStyle(ButtonStyle.Danger),
            );
            ...
            collector.on('collect', async buttonInteraction => {
                if (!buttonInteraction.member.roles.cache.has(APPROVER_ROLE_ID)) {
                    await buttonInteraction.reply({
                        content: 'You do not have permission to approve or reject recruitment logs.',
                        ephemeral: true,
                    });
                    return;
                }

                if (buttonInteraction.customId === 'recruitmentlog_approve') {
                    try {
                        await ensureTable();
                        const db = await getConnection();

                        await db.query(
                            `INSERT INTO recruitment_logs (recruit_id, submitted_by, approved_by)
                             VALUES (?, ?, ?)`,
                            [recruit.id, submitter.id, buttonInteraction.user.id]
                        );
                    } catch (err) {
                        console.error(err);
                        await buttonInteraction.reply({
                ...
                } else if (buttonInteraction.customId === 'recruitmentlog_reject') {
